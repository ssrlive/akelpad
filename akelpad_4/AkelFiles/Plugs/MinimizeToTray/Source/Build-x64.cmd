...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"MinimizeToTray.res" "Resources\MinimizeToTray.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% MinimizeToTray.c MinimizeToTray.res /LD /link kernel32.lib user32.lib shell32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist MinimizeToTray.res del MinimizeToTray.res
if exist MinimizeToTray.obj del MinimizeToTray.obj
if exist MinimizeToTray.lib del MinimizeToTray.lib
if exist MinimizeToTray.exp del MinimizeToTray.exp
if exist MinimizeToTray.ilk del MinimizeToTray.ilk
