...
void UpdateContextMenu(POPUPMENU *hMenuStack, HMENU hSubMenu);
...
void InitMenuPopup(POPUPMENU *hMenuStack, HMENU hParentMenu, BOOL bRemove);
...
BOOL StackIsIncludeSpecial(SPECIALPARENT *lpSpecialParent, HMENU hSubMenu);
...
void __declspec(dllexport) Show(PLUGINDATA *pd)
{
    ...
    else if (nAction == DLLA_CONTEXTMENU_SHOWMAINMENU)
    {
      ...
      if (pPosX && pPosY)
      {
        ...
        if (bMenuMainEnable)
        {
          ...
          //UpdateContextMenu(lpCurrentMenuStack, NULL);
        }
  ...
  if (!pd->bOnStart)
  {
    ...
    if (bResult)
    {
      ...
      {
        FindRootSubMenuByName(&hMenuManualStack, wszSubMenuName, &hMenu);
      }
...
LRESULT CALLBACK NewMainProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  else if (uMsg == WM_INITMENUPOPUP)
  {
    ...
    {
      InitMenuPopup(lpCurrentMenuStack, (HMENU)wParam, TRUE);
      UpdateContextMenu(lpCurrentMenuStack, (HMENU)wParam);
      InitMenuPopup(lpCurrentMenuStack, (HMENU)wParam, FALSE);
    }
  }
...
LRESULT CALLBACK MainDlgProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    CopyWideStr(wszManualText, -1, &ct[TYPE_MANUAL].wpText);
    CopyWideStr(wszMainText, -1, &ct[TYPE_MAIN].wpText);
    CopyWideStr(wszEditText, -1, &ct[TYPE_EDIT].wpText);
    CopyWideStr(wszTabText, -1, &ct[TYPE_TAB].wpText);
    CopyWideStr(wszUrlText, -1, &ct[TYPE_URL].wpText);
    CopyWideStr(wszRecentFilesText, -1, &ct[TYPE_RECENTFILES].wpText);
    ...
    else if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL)
    {
      ...
      if (LOWORD(wParam) == IDOK)
      {
        ...
        for (i=0; i < TYPE_MAX; ++i)
        {
          ...
          if (i == TYPE_MANUAL)
          {
            CopyWideStr(ct[TYPE_MANUAL].wpText, -1, &wszManualText);
            if (!*wszManualText)
            {
              FreeWideStr(&wszManualText);
              wszManualText=GetDefaultMenu(STRID_DEFAULTMANUAL);
            }
          }
          else if (i == TYPE_MAIN)
          {
            CopyWideStr(ct[TYPE_MAIN].wpText, -1, &wszMainText);
            if (!*wszMainText)
            {
              FreeWideStr(&wszMainText);
          ...
          {
            CopyWideStr(ct[TYPE_EDIT].wpText, -1, &wszEditText);
            if (!*wszEditText)
            {
              FreeWideStr(&wszEditText);
          ...
          {
            CopyWideStr(ct[TYPE_TAB].wpText, -1, &wszTabText);
            if (!*wszTabText)
            {
              FreeWideStr(&wszTabText);
          ...
          {
            CopyWideStr(ct[TYPE_URL].wpText, -1, &wszUrlText);
            if (!*wszUrlText)
            {
              FreeWideStr(&wszUrlText);
          ...
          {
            CopyWideStr(ct[TYPE_RECENTFILES].wpText, -1, &wszRecentFilesText);
            if (!*wszRecentFilesText)
            {
              FreeWideStr(&wszRecentFilesText);
...
void UpdateContextMenu(POPUPMENU *hMenuStack, HMENU hSubMenu)
{
  ...
  MENUITEM *lpMenuItem;
  SPECIALPARENT *lpSpecialParent=NULL;
  SPECIALPARENT *lpInitSpecialParent=NULL;
  STATEIF *lpStateIf;
  EXTPARAM *lpParameter;
  ...
  lpInitSpecialParent=StackGetSpecialParent(&hInitMenuStack->hSpecialMenuStack, hSubMenu);
  ...
  for (lpMenuItem=hMenuStack->hMenuItemStack.first; lpMenuItem; lpMenuItem=lpMenuItem->next)
  {
    if (lpMenuItem->hSubMenu == hSubMenu ||
        StackIsIncludeSpecial(lpSpecialParent, lpMenuItem->hSubMenu) ||
        StackIsIncludeSpecial(lpInitSpecialParent, lpMenuItem->hSubMenu))
    {
...
int ShowContextMenu(POPUPMENU *hMenuStack, HWND hWnd, int nType, int x, int y, HMENU hMenu)
{
  ...
  //UpdateContextMenu(hMenuStack, NULL);
    ...
    else
    {
      ...
      for (lpSpecialMenuItem=lpSpecialParent->first; lpSpecialMenuItem; lpSpecialMenuItem=lpSpecialMenuItem->next)
      {
        ...
        else if (lpSpecialMenuItem->nType == SI_INCLUDE)
        {
          if (lpSpecialMenuItem->lpIconMenuItem)
          {
            nAdded=IncludeMenu(hMenuStack->hIconMenu, hParentMenu, lpSpecialMenuItem->nFirstIndex, lpSpecialMenuItem->lpIconMenuItem);
...
BOOL StackIsIncludeSpecial(SPECIALPARENT *lpSpecialParent, HMENU hSubMenu)
{
  ...
  if (lpSpecialParent)
  {
    for (lpSpecialMenuItem=lpSpecialParent->first; lpSpecialMenuItem; lpSpecialMenuItem=lpSpecialMenuItem->next)
    {
      if (lpSpecialMenuItem->nType == SI_INCLUDE)
      {
        if (hSubMenu == (HMENU)lpSpecialMenuItem->lpIconMenuItem->nItemID)
          return TRUE;
      }
    }
  }
...
int IncludeMenu(HICONMENU hIconMenuDst, HMENU hMenuDst, int nPositionDst, ICONMENUITEM *lpMenuItemSrc)
{
  ...
  if (!(lpMenuItemSrc->uFlags & MF_POPUP))
    return 0;
  if (!(lpSubMenu=IconMenu_GetMenuByHandle(((ICONMENUSUBMENU *)(lpMenuItemSrc->hIconSubMenu))->hIconMenu, (HMENU)lpMenuItemSrc->nItemID)))
    return 0;

  for (lpMenuItemSrc=lpSubMenu->first; lpMenuItemSrc; lpMenuItemSrc=lpMenuItemSrc->next)
  {
    if (!(lpMenuItemSrc->uFlags & MF_POPUP))
      dwState=GetMenuState(lpSubMenu->hMenu, (UINT)lpMenuItemSrc->nItemID, MF_BYCOMMAND);
    else
      dwState=0;
...
void ReadOptions(DWORD dwFlags)
{
  ...
  if (hOptions=(HANDLE)SendMessage(hMainWnd, AKD_BEGINOPTIONSW, POB_READ, (LPARAM)wszPluginName))
  {
    ...
    {
      if (wszManualText=(wchar_t *)GlobalAlloc(GMEM_FIXED, nSize))
      {
    ...
    {
      if (wszMainText=(wchar_t *)GlobalAlloc(GMEM_FIXED, nSize))
      {
    ...
    {
      if (wszEditText=(wchar_t *)GlobalAlloc(GMEM_FIXED, nSize))
      {
    ...
    {
      if (wszTabText=(wchar_t *)GlobalAlloc(GMEM_FIXED, nSize))
      {
    ...
    {
      if (wszUrlText=(wchar_t *)GlobalAlloc(GMEM_FIXED, nSize))
      {
    ...
    {
      if (wszRecentFilesText=(wchar_t *)GlobalAlloc(GMEM_FIXED, nSize))
      {
