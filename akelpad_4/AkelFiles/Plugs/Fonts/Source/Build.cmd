...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Fonts.res" "Resources\Fonts.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Fonts.c Fonts.res /LD /link kernel32.lib user32.lib gdi32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist Fonts.res del Fonts.res
if exist Fonts.obj del Fonts.obj
if exist Fonts.lib del Fonts.lib
if exist Fonts.exp del Fonts.exp
if exist Fonts.ilk del Fonts.ilk
