...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Coder.res" "Resources\Coder.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Coder.c HighLight.c CodeFold.c AutoComplete.c Coder.res /LD /link kernel32.lib user32.lib gdi32.lib comdlg32.lib comctl32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
