...
BOOL ReadIni(INIFILE *hIniFile, HANDLE hFile)
{
  ...
  if ((dwFileSize=API_GetFileSize(hFile)) != INVALID_FILE_SIZE)
  {
    if (dwFileSize >= sizeof(wchar_t))
    {
      ...
      {
        if (API_ReadFile(hFile, wpText, dwFileSize, &dwBytesRead, NULL))
        {
      ...
      if (API_ReadFile(lpData->hFile, szBuffer, lpData->dwBytesMax, &dwBytesRead, NULL))
      {
        ...
        else
        {
          if (lpData->nCodePage == CP_UNICODE_UTF8)
            ...
            dwCharsConverted=API_MultiByteToWideChar(lpData->nCodePage, 0, (char *)szBuffer, dwBytesRead, wszBuffer, nBufferBytes / sizeof(wchar_t));
        }
      ...
      if (API_ReadFile(hFile, szBuffer, dwBytesMax, &dwBytesRead, NULL))
      {
        ...
        else
        {
          if (nCodePage == CP_UNICODE_UTF8)
            ...
            dwCharsConverted=API_MultiByteToWideChar(nCodePage, 0, (char *)szBuffer, dwBytesRead, wszBuffer, dwBufferBytes / sizeof(wchar_t));
        }
        wszBuffer[dwCharsConverted]='\0';
      }
    }
...
BOOL ColumnPaste(HWND hWnd)
{
  ...
  crInitialSel.ciMin=crSel.ciMin;
  crInitialSel.ciMax=crSel.ciMax;
  nLineSelStart=(int)SendMessage(hWnd, AEM_GETUNWRAPLINE, crSel.ciMin.nLine, 0);
  nLineSelEnd=(int)SendMessage(hWnd, AEM_GETUNWRAPLINE, crSel.ciMax.nLine, 0);
        ...
        LOWORD(wParam) == IDC_GOTO_OFFSET)
    {
      ...
      if (dwGotoType & GT_LINE)
      {
        if (!SendMessage(hWndNumber, EM_GETMODIFY, 0, 0))
        {
          //Caret line
          if (!(moCur.dwStatusPosType & SPT_LINEWRAP) && lpFrameCurrent->ei.bWordWrap)
            nLine=(int)SendMessage(lpFrameCurrent->ei.hWndEdit, AEM_GETUNWRAPLINE, ciCaret.nLine, 0);
          else
            nLine=ciCaret.nLine;
          ...
          if (moCur.dwStatusPosType & SPT_COLUMN)
            nColumn=(int)SendMessage(lpFrameCurrent->ei.hWndEdit, AEM_INDEXTOCOLUMN, MAKELONG(lpFrameCurrent->nTabStopSize, !(moCur.dwStatusPosType & SPT_LINEWRAP)), (LPARAM)&ciCaret);
          else
            nColumn=(int)SendMessage(lpFrameCurrent->ei.hWndEdit, AEM_INDEXTOCOLUMN, MAKELONG(1, !(moCur.dwStatusPosType & SPT_LINEWRAP)), (LPARAM)&ciCaret);
...
void RecodeTextW(HWND hWnd, int nCodePageFrom, int nCodePageTo)
{
  ...
  {
    nAnsiLen=API_WideCharToMultiByte(nCodePageFrom, 0, wszSelText, nUnicodeLen + 1, NULL, 0, NULL, NULL);

    if (szText=(char *)API_HeapAlloc(hHeap, 0, nAnsiLen))
    {
      API_WideCharToMultiByte(nCodePageFrom, 0, wszSelText, nUnicodeLen + 1, szText, nAnsiLen, NULL, NULL);
      nUnicodeLen=API_MultiByteToWideChar(nCodePageTo, 0, szText, nAnsiLen, NULL, 0);

      if (wszText=AllocWideStr(nUnicodeLen))
      {
        API_MultiByteToWideChar(nCodePageTo, 0, szText, nAnsiLen, wszText, nUnicodeLen);
...
void SetSelectionStatus(AEHDOC hDocEdit, HWND hWndEdit, AECHARRANGE *cr, AECHARINDEX *ci)
{
  if (lpFrameCurrent->ei.hDocEdit == hDocEdit)
  {
    ...
    if (!(moCur.dwStatusPosType & SPT_LINEWRAP) && lpFrameCurrent->ei.bWordWrap)
      nLine=(int)SendMessage(hWndEdit, AEM_GETUNWRAPLINE, (WPARAM)ciCaret.nLine, 0);
    else
      nLine=ciCaret.nLine;

    if (moCur.dwStatusPosType & SPT_COLUMN)
      nColumn=(int)SendMessage(hWndEdit, AEM_INDEXTOCOLUMN, MAKELONG(lpFrameCurrent->nTabStopSize, !(moCur.dwStatusPosType & SPT_LINEWRAP)), (LPARAM)&ciCaret);
    else
      nColumn=(int)SendMessage(hWndEdit, AEM_INDEXTOCOLUMN, MAKELONG(1, !(moCur.dwStatusPosType & SPT_LINEWRAP)), (LPARAM)&ciCaret);
