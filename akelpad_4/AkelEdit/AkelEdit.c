...
LRESULT CALLBACK AE_EditProc(AKELEDIT *ae, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg >= WM_USER)
  {
    ...
    if (uMsg == AEM_GETINDEX)
    {
      ...
      return (LRESULT)AE_GetIndex(ae, (int)wParam, ciCharIndex, ciCharIndex, FALSE);
    }
    ...
    if (uMsg == AEM_SETRECT)
    {
      ...
      AE_SetDrawRect(ae, rcDraw, (BOOL)wParam);
    ...
    if (uMsg == AEM_CONVERTPOINT)
    {
      ...
      {
        AE_GlobalToClient(ae, pt, &ptClient);
        pt->x=ptClient.x;
        pt->y=ptClient.y;
      }
      if (wParam == AECPT_CLIENTTOGLOBAL)
      {
        ptClient.x=(int)pt->x;
        ptClient.y=(int)pt->y;
        AE_ClientToGlobal(ae, &ptClient, pt);
      }
      return 0;
    }
      ...
      uMsg == EM_REPLACESELW)
  {
    if (uMsg == EM_REPLACESELA || (!ae->bUnicodeWindow && uMsg == EM_REPLACESEL))
      AE_ReplaceSelAnsi(ae, CP_ACP, (char *)lParam, (UINT_PTR)-1, AELB_ASINPUT, FALSE, NULL, NULL);
    else
...
void AE_GlobalToClient(AKELEDIT *ae, const POINT64 *ptGlobal, POINT *ptClient)
{
  ptClient->x=ae->rcDraw.left + (int)(ptGlobal->x - ae->nHScrollPos);
  ptClient->y=ae->rcDraw.top + (int)(ptGlobal->y - ae->nVScrollPos);
}

void AE_ClientToGlobal(AKELEDIT *ae, const POINT *ptClient, POINT64 *ptGlobal)
{
...
BOOL AE_IsPointOnSelection(AKELEDIT *ae, const POINT *ptPos)
{
  ...
  if (AEC_IndexCompare(&ae->ciSelStartIndex, &ae->ciSelEndIndex))
  {
    nSelStartY=(int)(AE_VPos(ae, ae->ciSelStartIndex.nLine, AEVPF_VPOSFROMLINE) - ae->nVScrollPos) + ae->rcDraw.top;
    nSelEndY=(int)(AE_VPos(ae, ae->ciSelEndIndex.nLine + 1, AEVPF_VPOSFROMLINE) - ae->nVScrollPos) + ae->rcDraw.top;

    if (ptPos->y >= nSelStartY && ptPos->y <= nSelEndY)
    {
      if (nResult=AE_GetCharFromPos(ae, ptPos, &ciCharIndex, &ptGlobal, ae->bColumnSel))
      {
        ...
        if (ciCharIndex.nLine >= ae->ciSelStartIndex.nLine &&
            ...
            ciCharIndex.nCharInLine == ciCharIndex.lpLine->nLineLen)
        {
          AE_GlobalToClient(ae, &ptGlobal, &ptClient);

          if (ptPos->x >= ptClient.x && ptPos->x <= ptClient.x + ae->ptxt->nAveCharWidth)
            return TRUE;
        }
      }
    }
      ...
      while (ciCount.nCharInLine <= ciCount.lpLine->nLineLen)
      {
        if (!qm->lpQuote)
        {
          ...
          for (lpQuoteElement=lpQuoteStack->first; lpQuoteElement; lpQuoteElement=lpQuoteElement->next)
          {
            ...
            {
              if ((lpDelimiterElement=AE_HighlightIsDelimiter(ae, &ft, &ciCount, FALSE)) || AEC_IsFirstCharInLine(&ciCount))
              {
                ...
                if (AE_IsMatch(ae, &ft, &ciCount))
                {
                  if (!AE_IsEscaped(&ciCount, lpQuoteElement->chEscape))
                  {
                    if (!(lpQuoteElement->dwFlags & AEHLF_QUOTESTART_ISWORD) ||
                        ((AE_HighlightIsDelimiter(ae, NULL, &ft.crFound.ciMax, FALSE) || AEC_IsLastCharInLine(&ft.crFound.ciMax)) &&
        ...
        else
        {
          ...
          {
            if ((lpDelimiterElement=AE_HighlightIsDelimiter(ae, &ft, &ciCount, FALSE)) || AEC_IsLastCharInLine(&ciCount))
            {
              if (lpDelimiterElement)
              {
...
AEDELIMITEMW* AE_HighlightIsDelimiter(AKELEDIT *ae, AEFINDTEXTW *ft, const AECHARINDEX *ciChar, BOOL bBack)
{
  ...
  while (lpDelimiterElement)
  {
    ...
    if (!bBack || AE_IndexOffset(ae, ciChar, &ciDelimStart, -(INT_PTR)ft->dwTextLen, ft->nNewLine) == ft->dwTextLen)
    {
...
AEWORDITEMW* AE_HighlightInsertWord(AKELEDIT *ae, AETHEMEITEMW *aeti, int nWordLen)
{
  ...
  if ((DWORD)nWordLen < sizeof(lpWordStack->lpWordLens) / sizeof(INT_PTR))
  {
    ...
    {
      lpTmp=(AEWORDITEMW *)lpWordStack->first;
...
void AE_ScrollToCaret(AKELEDIT *ae, const POINT64 *ptCaret, BOOL bVertCorrect)
{
  ...
  else
  {
    ...
    {
      if (bVertCorrect && AEC_IndexCompare(&ae->ciSelStartIndex, &ae->ciSelEndIndex) && !AEC_IndexCompare(&ae->ciCaretIndex, &ae->ciSelEndIndex))
...
DWORD AE_ScrollToPointEx(AKELEDIT *ae, DWORD dwFlags, POINT64 *ptPosition, int nOffsetX, int nOffsetY)
{
  ...
  {
    ptClient.x=(int)ptPosition->x;
    ptClient.y=(int)ptPosition->y;
    AE_ClientToGlobal(ae, &ptClient, &ptPos);
  }
  ...
  if (dwFlags & AESC_POINTOUT)
  {
    if (ptPosition)
    {
      ...
      {
        AE_GlobalToClient(ae, &ptPos, &ptClient);
        ptPosition->x=ptClient.x;
        ptPosition->y=ptClient.y;
      }
    }
  }
  return dwResult;
...
BOOL AE_PrintPage(AKELEDIT *ae, AEPRINTHANDLE *ph, AEPRINT *prn)
{
  ...
  while (ciCount.lpLine)
  {
    ...
    {
      if (AE_GetIndex(&ph->aePrint, (ae->popt->dwOptions & AECO_NOPRINTCOLLAPSED)?AEGI_NEXTUNCOLLAPSEDLINE:AEGI_NEXTLINE, &ciCount, &prn->crText.ciMin, FALSE))
        bContinuePrint=TRUE;
      else
        bContinuePrint=FALSE;
      goto PrintLine;
...
void AE_Paint(AKELEDIT *ae)
{
  ...
  if (GetUpdateRect(ae->hWndEdit, &rcErase, FALSE))
  {
    ...
    if (BeginPaint(ae->hWndEdit, &ps))
    {
      ...
      {
        to.ciDrawLine.nLine=(int)AE_VPos(ae, ae->nVScrollPos + (rcDraw.top - ae->rcDraw.top), AEVPF_LINEFROMVPOS);
        ...
        to.ptFirstCharInLine.y=(int)(AE_VPos(ae, to.ciDrawLine.nLine, AEVPF_VPOSFROMLINE) - ae->nVScrollPos) + ae->rcDraw.top;
      ...
      while (to.ciDrawLine.lpLine)
      {
          ...
          while (to.ciDrawLine.nCharInLine <= to.ciDrawLine.lpLine->nLineLen)
          {
            ...
            if (nLineSelection == AELS_FULL || nLineSelection == AELS_PARTLY)
            {
              if (to.ciDrawLine.lpLine->nSelStart == to.ciDrawLine.nCharInLine ||
                  ((hlp.dwFindFirst & AEHPT_SELECTION) && AEC_IsCharInSelection(&to.ciDrawLine)))
              {
        ...
        {
          if (AE_GetIndex(ae, AEGI_NEXTUNCOLLAPSEDLINE, &to.ciDrawLine, &to.ciDrawLine, FALSE))
...
void AE_RedrawLineRange(AKELEDIT *ae, int nFirstLine, int nLastLine, BOOL bErase)
{
  ...
  if (nFirstLine == -1)
    ...
    rcRedraw.top=(int)(AE_VPos(ae, nFirstLine, AEVPF_VPOSFROMLINE) - ae->nVScrollPos) + ae->rcDraw.top;
  if (nLastLine == -1)
    rcRedraw.bottom=ae->rcDraw.bottom;
  else
    rcRedraw.bottom=(int)(AE_VPos(ae, nLastLine, AEVPF_VPOSFROMLINE) + ae->ptxt->nCharHeight - ae->nVScrollPos) + ae->rcDraw.top;
...
int AE_GetFirstVisibleLine(AKELEDIT *ae)
{
  ...
  nFirstLine=(int)AE_VPos(ae, ae->nVScrollPos, AEVPF_LINEFROMVPOS);
...
int AE_GetLastVisibleLine(AKELEDIT *ae)
{
  ...
  nLastLine=(int)AE_VPos(ae, ae->nVScrollPos + (ae->rcDraw.bottom - ae->rcDraw.top), AEVPF_LINEFROMVPOS);
...
BOOL AE_GetPosFromChar(AKELEDIT *ae, const AECHARINDEX *ciCharIndex, POINT64 *ptGlobalPos, POINT *ptClientPos)
{
  ...
  if (ciCharIndex->lpLine)
  {
    ...
    if (ptClientPos)
    {
      ...
      ptClientPos->y=(int)(AE_VPos(ae, ciInitial.nLine, AEVPF_VPOSFROMLINE) - ae->nVScrollPos) + ae->rcDraw.top;
    }
    return TRUE;
  }
  return FALSE;
...
int AE_GetCharFromPos(AKELEDIT *ae, const POINT *ptClientPos, AECHARINDEX *ciCharIndex, POINT64 *ptGlobalPos, BOOL bColumnSel)
{
  INT_PTR nCharPos;
  INT_PTR nMaxExtent;
  int nFirstLine=0;
  ...
  ciCharIndex->nLine=(int)AE_VPos(ae, ae->nVScrollPos + (ptClientPos->y - ae->rcDraw.top), AEVPF_LINEFROMVPOS);
          ...
          else
          {
            ...
            else
            {
              ...
              while (lpElement)
              {
                ...
                {
                  //Middle
                  dwCharsToCopy=min((DWORD)lpElement->nLineLen, dwBufferSize - dwCopied);
                  xmemcpy(wszBuffer + dwCopied, lpElement->wpLine, dwCharsToCopy * sizeof(wchar_t));
                  dwCopied+=dwCharsToCopy;
                  if (dwCopied >= dwBufferSize) goto End;
                }
       ...
       ae->rcDraw.right - ae->rcDraw.left > 0)
  {
    ...
    else if (nVk == VK_PRIOR)
    {
      ...
      else
      {
        ...
        ciCharOut.nLine=(int)AE_VPos(ae, nVertPos, AEVPF_LINEFROMVPOS);
