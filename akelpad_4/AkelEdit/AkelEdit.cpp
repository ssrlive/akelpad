  ...
  if (ae=AE_StackWindowGet(&hAkelEditWindowsStack, hWnd))
  {
    ...
    if (uMsg >= WM_USER)
    {
      ...
      if (uMsg == AEM_HLADDQUOTEA)
      {
        ...
        if (lpQuoteDst=AE_HighlightInsertQuote(ae, lpTheme))
        {
      ...
      if (uMsg == AEM_HLADDMARKTEXTA)
      {
        ...
        if (lpMarkTextDst=AE_HighlightInsertMarkText(ae, lpTheme))
        {
      ...
      if (uMsg == AEM_HLADDMARKTEXTW)
      {
        ...
        {
          if (lpMarkTextDst->pMarkText=(wchar_t *)AE_HeapAlloc(NULL, 0, lpMarkTextSrc->nMarkTextLen * sizeof(wchar_t) + 2))
            AE_memcpy(lpMarkTextDst->pMarkText, lpMarkTextSrc->pMarkText, lpMarkTextSrc->nMarkTextLen * sizeof(wchar_t) + 2);
          lpMarkTextDst->nMarkTextLen=lpMarkTextSrc->nMarkTextLen;
      ...
      if (uMsg == AEM_HLADDMARKRANGE)
      {
        ...
        if (lpMarkRangeDst=AE_HighlightInsertMarkRange(ae, lpTheme))
        {
    ...
    if (uMsg == EM_FINDWORDBREAK)
    {
          ...
          wParam == WB_MOVEWORDPREV)
      {
        ...
        if (AE_GetPrevWord(ae, &ciCharIn, &ciCharOut, NULL, FALSE, dwFlags))
          ...
          wParam == WB_MOVEWORDNEXT)
      {
        ...
        if (AE_GetNextWord(ae, &ciCharIn, NULL, &ciCharOut, FALSE, dwFlags))
          return AE_AkelIndexToRichOffset(ae, &ciCharOut);
        else
          return lParam;
      }
          ...
          uMsg == WM_MBUTTONDOWN)
      {
        if (ae->bMButtonDown)
        {
          if (ae->dwMouseScrollTimer)
          {
            ...
            AE_ReleaseMouseCapture(ae, AEMC_MOUSESCROLL);
          }
          AE_MButtonErase(ae);
        ...
        if (ae->nLButtonDownCount == 0)
        {
          ...
          if (!bAlt && !bShift && ae->nCurrentCursor == AECC_MARGIN)
          {
            ...
            if (!ae->dwMouseMoveTimer)
            {
              ...
              AE_SetMouseCapture(ae, AEMC_MOUSEMOVE);
            }
        ...
        if (!ae->bMButtonDown)
        {
              ...
              ae->ptxt->nVScrollMax > ae->rcDraw.bottom - ae->rcDraw.top)
          {
            ...
            if (!ae->dwMouseScrollTimer)
            {
              ...
              AE_SetMouseCapture(ae, AEMC_MOUSESCROLL);
            }
          }
        }
        return 0;
...
int AE_GetIndex(AKELEDIT *ae, int nType, const AECHARINDEX *ciCharIn, AECHARINDEX *ciCharOut, BOOL bColumnSel)
{
  ...
  {
    return AE_GetNextWord(ae, ciCharIn, NULL, ciCharOut, bColumnSel, AEWB_RIGHTWORDSTART);
  }
  else if (nType == AEGI_NEXTWORDEND)
  {
    return AE_GetNextWord(ae, ciCharIn, NULL, ciCharOut, bColumnSel, AEWB_RIGHTWORDEND);
  }
  else if (nType == AEGI_PREVWORDSTART)
  {
    return AE_GetPrevWord(ae, ciCharIn, ciCharOut, NULL, bColumnSel, AEWB_LEFTWORDSTART);
  }
  else if (nType == AEGI_PREVWORDEND)
  {
    return AE_GetPrevWord(ae, ciCharIn, ciCharOut, NULL, bColumnSel, AEWB_LEFTWORDEND);
  }
  ...
  if (ae->rcDraw.bottom - ae->rcDraw.top > 0 && ae->rcDraw.right - ae->rcDraw.left > 0)
  {
    ...
    else if (ae->nLButtonDownCount == 1)
    {
      if (bShift)
      {
        ...
        {
          if (AE_IndexCompare(&ciCharIndex, &ae->ciLButtonStart) < 0)
            AE_GetPrevWord(ae, &ciCharIndex, &ciCharIndex, NULL, bColumnSel, ae->popt->dwWordBreak);
          else
            ciCharIndex=ae->ciLButtonStart;
          ciSelEnd=ae->ciLButtonEnd;
        }
        ...
        {
          if (AE_IndexCompare(&ciCharIndex, &ae->ciLButtonEnd) > 0)
            AE_GetNextWord(ae, &ciCharIndex, NULL, &ciCharIndex, bColumnSel, ae->popt->dwWordBreak);
          else
            ciCharIndex=ae->ciLButtonEnd;
          ciSelEnd=ae->ciLButtonStart;
        }
...
AEMARKTEXTITEMW* AE_HighlightInsertMarkText(AKELEDIT *ae, AETHEMEITEMW *aeti)
{
  ...
  AE_HeapStackInsertIndex(NULL, (stack **)&aeti->hMarkTextStack.first, (stack **)&aeti->hMarkTextStack.last, (stack **)&lpElement, -1, sizeof(AEMARKTEXTITEMW));
  return lpElement;
}
...
AEMARKRANGEITEM* AE_HighlightInsertMarkRange(AKELEDIT *ae, AETHEMEITEMW *aeti)
{
  ...
  AE_HeapStackInsertIndex(NULL, (stack **)&aeti->hMarkRangeStack.first, (stack **)&aeti->hMarkRangeStack.last, (stack **)&lpElement, -1, sizeof(AEMARKRANGEITEM));
  return lpElement;
}
  ...
  if (GetUpdateRect(ae->hWndEdit, &ae->rcErase, FALSE))
  {
    if (BeginPaint(ae->hWndEdit, &ps))
    {
          ...
          rcDraw.bottom > ae->rcDraw.top)
      {
        ...
        nDrawCharOffset=ae->nFirstDrawLineOffset;
      }
      ...
      while (ciDrawLine.lpLine)
      {
        ...
        dwFindFirst=AEPT_SELECTION|AEPT_MARKTEXT|AEPT_LINK|AEPT_QUOTE|AEPT_DELIM1;
        ...
        while (ciDrawLine.nCharInLine <= ciDrawLine.lpLine->nLineLen)
        {
          ...
          if (ae->popt->lpActiveTheme)
          {
            ...
            if (hlp.mrm.lpMarkRange)
            {
              if (hlp.mrm.crMarkRange.cpMax <= nDrawCharOffset)
              {
                ...
                hlp.dwPaintType&=~AEPT_MARKRANGE;
          ...
          if (ae->popt->lpActiveTheme)
          {
            ...
            if (hlp.mrm.lpMarkRange)
            {
              if (!(hlp.dwPaintType & AEPT_MARKRANGE))
              {
              ...
              if (!(hlp.dwPaintType & AEPT_SELECTION) && (hlp.dwPaintType & AEPT_MARKRANGE))
              {
            ...
            if (AE_IndexCompare(&hlp.mtm.crMarkText.ciMax, &ciDrawLine) <= 0)
            {
              if (dwFindFirst & AEPT_MARKTEXT)
              {
                dwFindFirst&=~AEPT_MARKTEXT;
            ...
            if (hlp.mtm.lpMarkText)
            {
              if (!(hlp.dwPaintType & AEPT_MARKTEXT))
              {
              ...
              if (!(hlp.dwPaintType & AEPT_SELECTION) && (hlp.dwPaintType & AEPT_MARKTEXT))
              {
          ...
          nDrawCharOffset-=ciDrawLine.nCharInLine;
...
BOOL AE_GetPrevWord(AKELEDIT *ae, const AECHARINDEX *ciChar, AECHARINDEX *ciWordStart, AECHARINDEX *ciWordEnd, BOOL bColumnSel, DWORD dwFlags)
{
  ...
  if (ciStart.nCharInLine - 1 < 0)
    ...
    wchChar=ciStart.lpLine->wpLine[ciStart.nCharInLine - 1];
       ...
       ae->rcDraw.right - ae->rcDraw.left > 0)
  {
    ...
    else if (nVk == VK_LEFT)
    {
      if (bControl)
      {
        ...
        AE_GetPrevWord(ae, &ciCharIn, &ciCharOut, NULL, bAlt, ae->popt->dwWordBreak);
      }
      else
      {
        if (!bShift && AE_IndexCompare(&ae->ciSelStartIndex, &ae->ciSelEndIndex))
    ...
    else if (nVk == VK_RIGHT)
    {
      if (bControl)
      {
        ...
        AE_GetNextWord(ae, &ciCharIn, NULL, &ciCharOut, bAlt, ae->popt->dwWordBreak);
      }
      else
      {
        if (!bShift && AE_IndexCompare(&ae->ciSelStartIndex, &ae->ciSelEndIndex))
...
void AE_EditKeyBackspace(AKELEDIT *ae, BOOL bControl)
{
  ...
  if (!AE_IndexCompare(&ae->ciSelStartIndex, &ae->ciSelEndIndex))
  {
    ...
    if ((bControl && AE_GetPrevWord(ae, &ae->ciSelStartIndex, &ciCharIndex, NULL, FALSE, ae->popt->dwWordBreak)) ||
...
void AE_EditKeyDelete(AKELEDIT *ae, BOOL bControl)
{
  ...
  if (!AE_IndexCompare(&ae->ciSelStartIndex, &ae->ciSelEndIndex))
  {
    ...
    if ((bControl && AE_GetNextWord(ae, &ae->ciSelStartIndex, NULL, &ciCharIndex, FALSE, ae->popt->dwWordBreak)) ||
