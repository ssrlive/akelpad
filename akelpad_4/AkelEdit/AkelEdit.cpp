  ...
  if (ae=AE_StackWindowGet(&hAkelEditWindowsStack, hWnd))
  {
    ...
    if (uMsg >= WM_USER)
    {
      ...
      if (uMsg == AEM_HLCREATETHEMEA)
      {
        ...
        return (LRESULT)AE_HighlightCreateTheme(ae, wszThemeName);
      }
      ...
      if (uMsg == AEM_HLGETTHEMEA)
      {
        ...
        MultiByteToWideChar(CP_ACP, 0, pThemeName, -1, wszThemeName, MAX_PATH);
        return (LRESULT)AE_HighlightGetTheme(ae, wszThemeName);
      }
      ...
      if (uMsg == AEM_HLGETDELIMITERA)
      {
        AETHEMEITEMW *lpTheme=(AETHEMEITEMW *)wParam;
        AEDELIMITEMA *lpDelimDst=(AEDELIMITEMA *)lParam;
        AEDELIMITEMW *lpDelimSrc=NULL;
        wchar_t *wpDelimiter;

        if (wpDelimiter=(wchar_t *)AE_HeapAlloc(NULL, 0, lpDelimDst->nDelimiterLen * sizeof(wchar_t) + 2))
        {
          MultiByteToWideChar(CP_ACP, 0, lpDelimDst->pDelimiter, lpDelimDst->nDelimiterLen + 1, wpDelimiter, lpDelimDst->nDelimiterLen + 1);

          if (lpDelimSrc=AE_HighlightGetDelimiter(ae, lpTheme, wpDelimiter, lpDelimDst->nDelimiterLen, lpDelimDst->dwFlags))
          {
            lpDelimDst->crText=lpDelimSrc->crText;
            lpDelimDst->crBk=lpDelimSrc->crBk;
            lpDelimDst->dwFontStyle=lpDelimSrc->dwFontStyle;
          }
          AE_HeapFree(NULL, 0, (LPVOID)wpDelimiter);
        }
        return (LRESULT)lpDelimSrc;
      }
      if (uMsg == AEM_HLGETDELIMITERW)
      {
        AETHEMEITEMW *lpTheme=(AETHEMEITEMW *)wParam;
      ...
      if (uMsg == AEM_HLGETWORDA)
      {
        AETHEMEITEMW *lpTheme=(AETHEMEITEMW *)wParam;
        AEWORDITEMA *lpWordDst=(AEWORDITEMA *)lParam;
        AEWORDITEMW *lpWordSrc=NULL;
        wchar_t *wpWord;

        if (wpWord=(wchar_t *)AE_HeapAlloc(NULL, 0, lpWordDst->nWordLen * sizeof(wchar_t) + 2))
        {
          MultiByteToWideChar(CP_ACP, 0, lpWordDst->pWord, lpWordDst->nWordLen + 1, wpWord, lpWordDst->nWordLen + 1);

          if (lpWordSrc=AE_HighlightGetWord(ae, lpTheme, wpWord, lpWordDst->nWordLen, lpWordDst->dwFlags))
          {
            lpWordDst->crText=lpWordSrc->crText;
            lpWordDst->crBk=lpWordSrc->crBk;
            lpWordDst->dwFontStyle=lpWordSrc->dwFontStyle;
          }
          AE_HeapFree(NULL, 0, (LPVOID)wpWord);
        }
        return (LRESULT)lpWordSrc;
      }
      if (uMsg == AEM_HLGETWORDW)
      {
        AETHEMEITEMW *lpTheme=(AETHEMEITEMW *)wParam;
      ...
      if (uMsg == AEM_HLGETQUOTEA)
      {
        AETHEMEITEMW *lpTheme=(AETHEMEITEMW *)wParam;
        AEQUOTEITEMA *lpQuoteDst=(AEQUOTEITEMA *)lParam;
        AEQUOTEITEMW *lpQuoteSrc=NULL;
        wchar_t *wpQuoteStart;

        if (wpQuoteStart=(wchar_t *)AE_HeapAlloc(NULL, 0, lpQuoteDst->nQuoteStartLen * sizeof(wchar_t) + 2))
        {
          MultiByteToWideChar(CP_ACP, 0, lpQuoteDst->pQuoteStart, lpQuoteDst->nQuoteStartLen + 1, wpQuoteStart, lpQuoteDst->nQuoteStartLen + 1);

          if (lpQuoteSrc=AE_HighlightGetQuote(ae, lpTheme, wpQuoteStart, lpQuoteDst->nQuoteStartLen, lpQuoteDst->dwFlags))
          {
            lpQuoteDst->crText=lpQuoteSrc->crText;
            lpQuoteDst->crBk=lpQuoteSrc->crBk;
            lpQuoteDst->dwFontStyle=lpQuoteSrc->dwFontStyle;
          }
          AE_HeapFree(NULL, 0, (LPVOID)wpQuoteStart);
        }
        return (LRESULT)lpQuoteSrc;
      }
      if (uMsg == AEM_HLGETQUOTEW)
      {
        AETHEMEITEMW *lpTheme=(AETHEMEITEMW *)wParam;
...
int AE_GetIndex(AKELEDIT *ae, int nType, const AECHARINDEX *ciCharIn, AECHARINDEX *ciCharOut, BOOL bColumnSel)
{
  ...
  {
    return AE_GetNextWord(ae, ciCharIn, NULL, ciCharOut, bColumnSel, AEWB_RIGHTWORDSTART, 0);
  }
  else if (nType == AEGI_NEXTWORDEND)
  {
    return AE_GetNextWord(ae, ciCharIn, NULL, ciCharOut, bColumnSel, AEWB_RIGHTWORDEND, 0);
  }
  else if (nType == AEGI_PREVWORDSTART)
  {
    return AE_GetPrevWord(ae, ciCharIn, ciCharOut, NULL, bColumnSel, AEWB_LEFTWORDSTART, 0);
  }
  else if (nType == AEGI_PREVWORDEND)
  {
    return AE_GetPrevWord(ae, ciCharIn, ciCharOut, NULL, bColumnSel, AEWB_LEFTWORDEND, 0);
  }
...
int AE_IndexDec(AECHARINDEX *ciChar)
{
  if (ciChar->nCharInLine - 2 >= 0)
  {
    ...
    {
      if (IsLowSurrogate(ciChar->lpLine->wpLine[ciChar->nCharInLine - 1]))
        if (IsHighSurrogate(ciChar->lpLine->wpLine[ciChar->nCharInLine - 2]))
          --ciChar->nCharInLine;
    }
  }
  return --ciChar->nCharInLine;
...
BOOL AE_IndexNormalize(AECHARINDEX *ciChar)
{
  ...
  {
    if (IsLowSurrogate(ciChar->lpLine->wpLine[ciChar->nCharInLine]))
    {
      if (IsHighSurrogate(ciChar->lpLine->wpLine[ciChar->nCharInLine - 1]))
      {
        --ciChar->nCharInLine;
        return TRUE;
      }
...
void AE_HighlightDeleteThemeAll(AKELEDIT *ae)
{
  ...
  if (ae) ae->popt->lpActiveTheme=NULL;
...
AEDELIMITEMW* AE_HighlightInsertDelimiter(AKELEDIT *ae, AETHEMEITEMW *aeti, int nDelimiterLen)
{
  ...
  AE_HeapStackInsertIndex(NULL, (stack **)&aeti->hDelimiterStack.first, (stack **)&aeti->hDelimiterStack.last, (stack **)&lpElement, -1, sizeof(AEDELIMITEMW));
  return lpElement;
}
...
AEQUOTEITEMW* AE_HighlightInsertQuote(AKELEDIT *ae, AETHEMEITEMW *aeti, int nQuoteStartLen)
{
  ...
  AE_HeapStackInsertIndex(NULL, (stack **)&aeti->hQuoteStack.first, (stack **)&aeti->hQuoteStack.last, (stack **)&lpElement, -1, sizeof(AEQUOTEITEMW));
  return lpElement;
}
  ...
  if (GetUpdateRect(ae->hWndEdit, &ae->rcErase, FALSE))
  {
    if (BeginPaint(ae->hWndEdit, &ps))
    {
      ...
      while (ciDrawLine.lpLine)
      {
        ...
        AE_GetCharInLine(ae, ciDrawLine.lpLine, nMinPaintWidth - ae->ptxt->nAveCharWidth, AECLR_ALLPOS, &ciDrawLine.nCharInLine, &nLineWidth, FALSE);
        nStartDrawWidth=nLineWidth;
        nMaxDrawCharsCount=0;
        wpStartDraw=ciDrawLine.lpLine->wpLine + ciDrawLine.nCharInLine;
        dwFindFirst=AEPT_SELECTION|AEPT_LINK|AEPT_QUOTE|AEPT_DELIM1;
        ...
        //Scan line
        for (; ciDrawLine.nCharInLine <= ciDrawLine.lpLine->nLineLen; AE_IndexInc(&ciDrawLine))
        {
          ...
          if (nLineSelection == AELS_FULL || nLineSelection == AELS_PARTLY)
          {
            if (ciDrawLine.lpLine->nSelStart == ciDrawLine.nCharInLine ||
                ((dwFindFirst & AEPT_SELECTION) && AE_IsCharInSelection(&ciDrawLine)))
            {
              dwFindFirst&=~AEPT_SELECTION;
          ...
          if (ae->popt->bDetectUrl)
          {
            ...
            if (AE_IndexCompare(&hlp.crLink.ciMax, &ciDrawLine) <= 0)
            {
              if (dwFindFirst & AEPT_LINK)
              {
                dwFindFirst&=~AEPT_LINK;
          ...
          if (ae->popt->lpActiveTheme)
          {
            ...
            if (!(hlp.dwPaintType & AEPT_LINK))
            {
              ...
              {
                if (dwFindFirst & AEPT_QUOTE)
                {
                  dwFindFirst&=~AEPT_QUOTE;
              ...
              if (hlp.qm.lpQuote)
              {
                ...
                if (!(hlp.dwPaintType & AEPT_SELECTION) && (hlp.dwPaintType & AEPT_QUOTE))
                {
              ...
              if (!hlp.qm.lpQuote || AE_IndexCompare(&ciDrawLine, &hlp.qm.crQuoteStart.ciMin) < 0)
              {
                ...
                if (AE_IndexCompare(&hlp.wm.crDelim2.ciMax, &ciDrawLine) <= 0)
                {
                  if (dwFindFirst & AEPT_DELIM1)
                  {
                    dwFindFirst&=~AEPT_DELIM1;
                ...
                if (hlp.wm.lpDelim1)
                {
                  if (!(hlp.dwPaintType & AEPT_DELIM1))
                  {
                  ...
                  if (!(hlp.dwPaintType & AEPT_SELECTION) && (hlp.dwPaintType & AEPT_DELIM1))
                  {
                ...
                if (hlp.wm.lpWord)
                {
                  ...
                  if (!(hlp.dwPaintType & AEPT_SELECTION) && (hlp.dwPaintType & AEPT_WORD))
                  {
                ...
                if (hlp.wm.lpDelim2)
                {
                  if (!(hlp.dwPaintType & AEPT_DELIM2))
                  {
                  ...
                  if (!(hlp.dwPaintType & AEPT_SELECTION) && (hlp.dwPaintType & AEPT_DELIM2))
                  {
...
void AE_PaintTextOut(AKELEDIT *ae, HDC hDC, AEHLPAINT *hlp, const POINT *ptDraw, const wchar_t *wpLine, int nLineLen, int nLineWidth, wchar_t **wpTextInLine, int *nTextInLineWidth)
{
  ...
  if (nTextLen)
  {
    SetTextColor(hDC, hlp->dwActiveText);
    SetBkColor(hDC, hlp->dwActiveBG);
        ...
        rcTextOut.left < ae->rcDraw.right)
    {
      if (!(hlp->dwPaintType & AEPT_SELECTION))
      {
        if (hlp->dwPaintType & AEPT_LINK)
        {
          if (ae->ptxt->hFontUrl) SelectObject(hDC, ae->ptxt->hFontUrl);
        }
        if (hlp->dwFontStyle)
        {
          if (hlp->dwFontStyle == AEHLS_FONTNORMAL)
          {
            if (ae->ptxt->hFontNormal) SelectObject(hDC, ae->ptxt->hFontNormal);
          }
          else if (hlp->dwFontStyle == AEHLS_FONTBOLD)
          {
            if (ae->ptxt->hFontBold) SelectObject(hDC, ae->ptxt->hFontBold);
          }
          else if (hlp->dwFontStyle == AEHLS_FONTITALIC)
          {
            if (ae->ptxt->hFontItalic) SelectObject(hDC, ae->ptxt->hFontItalic);
          }
      ...
      if (!(ae->popt->dwOptions & AECO_PAINTGROUP))
      {
        ...
        {
          //Surrogate character
          if (IsSurrogate(wpText[i]))
          {
            if (i + 1 < nTextLen)
            {
              if (IsHighSurrogate(wpText[i]) && IsLowSurrogate(wpText[i + 1]))
              {
                TextOutW(hDC, rcTextOut.left, rcTextOut.top, wpText + i, 2);
          ...
          {
            TextOutW(hDC, rcTextOut.left, rcTextOut.top, wpText + i, 1);
            rcTextOut.left+=AE_GetCharWidth(ae, wpText + i, NULL);
          }
        }
      }
      else ExtTextOutW(hDC, rcTextOut.left, rcTextOut.top, dwTextOutFlags, &rcTextOut, wpText, nTextLen, NULL);
  ...
  for (i=0; i < nStringLen; ++i)
  {
    ...
    else
    {
      ...
      if (hDC)
      {
        ...
        {
          //Surrogate character
          if (IsSurrogate(wpString[i]))
          {
            if (i + 1 < nStringLen)
            {
              if (IsHighSurrogate(wpString[i]) && IsLowSurrogate(wpString[i + 1]))
              {
...
int AE_GetCharWidth(AKELEDIT *ae, wchar_t *wpChar, int *nDone)
{
  ...
  if (IsHighSurrogate(*wpChar))
  {
    if (IsLowSurrogate(*(wpChar + 1)))
      nCharCount=2;
    else
      nCharCount=0;
  }
  ...
  if (nCharCount)
    AE_GetTextExtentPoint32(ae, wpChar, nCharCount, &sizeChar);
  else
    sizeChar.cx=0;
  if (nDone) *nDone=nCharCount;
  return sizeChar.cx;
}
...
BOOL AE_GetCharRangeInLine(AKELEDIT *ae, AELINEDATA *lpLine, int nMinExtent, int nMaxExtent, int *nMinCharIndex, int *nMinCharPos, int *nMaxCharIndex, int *nMaxCharPos, BOOL bColumnSel)
{
  ...
  {
    if (!AE_GetCharInLine(ae, lpLine, nMinExtent, AECIL_HALFFIT|AECLR_ALLPOS, &nMinIndex, &nMinPos, bColumnSel))
      return FALSE;
  }
  if (nMaxCharIndex || nMaxCharPos)
  {
    if (!AE_GetCharInLine(ae, lpLine, nMaxExtent, AECIL_HALFFIT|AECLR_ALLPOS, &nMaxIndex, &nMaxPos, bColumnSel))
      return FALSE;
  }
...
int AE_GetCharFromPos(AKELEDIT *ae, const POINT *ptClientPos, AECHARINDEX *ciCharIndex, POINT *ptGlobalPos, BOOL bColumnSel)
{
  ...
  {
    if (nResult=AE_GetCharInLine(ae, ciCharIndex->lpLine, nMaxExtent, AECIL_HALFFIT|AECLR_ALLPOS, &ciCharIndex->nCharInLine, &nCharPos, bColumnSel))
    {
          ...
          if (lpNewElement=AE_StackLineInsertBefore(ae, ciInsertFrom.lpLine))
          {
            ...
            if (lpElement)
            {
              ...
              if (!ciFirstChar.lpLine)
                ...
                AE_GetCharInLine(ae, lpElement, ptInsertFrom.x, AECIL_HALFFIT|AECLR_ALLPOS, &ciInsertFrom.nCharInLine, NULL, bColumnSel);
