...
int ReplaceTextA(HWND hWnd, DWORD dwFlags, char *pFindIt, char *pReplaceWith, BOOL bAll, int *nReplaceCount)
{
  ...
  if (bAll)
  {
    ...
    {
      if (StrReplaceA(szText, pFindIt, pReplaceWith, (dwFlags & FR_MATCHCASE)?TRUE:FALSE, NULL, &nReplaceTextLen, NULL, NULL, NULL))
      {
        if (szReplaceText=(char *)API_HeapAlloc(hHeap, 0, nReplaceTextLen + 1))
        {
          ...
          if (nChanges=StrReplaceA(szText, pFindIt, pReplaceWith, (dwFlags & FR_MATCHCASE)?TRUE:FALSE, szReplaceText, NULL, &pMin, &pMax, &pFirstVisible))
          {
            SendMessage(hWnd, WM_SETREDRAW, FALSE, 0);
            SetTextSel(hWnd, crRange.cpMin, crRange.cpMax);
...
int ReplaceTextW(HWND hWnd, DWORD dwFlags, wchar_t *wpFindIt, wchar_t *wpReplaceWith, BOOL bAll, int *nReplaceCount)
{
  ...
  if (bAll)
  {
    ...
    {
      if (StrReplaceW(wszText, wpFindIt, wpReplaceWith, (dwFlags & FR_MATCHCASE)?TRUE:FALSE, NULL, &nReplaceTextLen, NULL, NULL, NULL))
      {
        if (wszReplaceText=(wchar_t *)API_HeapAlloc(hHeap, 0, nReplaceTextLen * sizeof(wchar_t) + 2))
        {
          ...
          if (nChanges=StrReplaceW(wszText, wpFindIt, wpReplaceWith, (dwFlags & FR_MATCHCASE)?TRUE:FALSE, wszReplaceText, NULL, &wpMin, &wpMax, &wpFirstVisible))
          {
            SendMessage(hWnd, WM_SETREDRAW, FALSE, 0);
            SetTextSel(hWnd, crRange.cpMin, crRange.cpMax);
...
BOOL CALLBACK GoToLineDlgProcA(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  else if (uMsg == WM_COMMAND)
  {
    if (LOWORD(wParam) == IDOK)
    {
      ...
      {
        nLineNumber=SendMessage(hWndEdit, EM_GETLINECOUNT, 0, 0) + nLineNumber + 1;
      ...
      nGotoLine=SendMessage(hWndEdit, EM_LINEINDEX, nLineNumber - 1, 0);
      if (nGotoLine == -1) nGotoLine=SendMessage(hWndEdit, EM_LINEINDEX, SendMessage(hWndEdit, EM_GETLINECOUNT, 0, 0) - 1, 0);
      nGotoLineLen=SendMessage(hWndEdit, EM_LINELENGTH, nGotoLine, 0);

      if (nColumn <= nGotoLineLen) nGotoLine+=nColumn - 1;
      else nGotoLine+=nGotoLineLen;
      SetTextSel(hWndEdit, nGotoLine, nGotoLine);
    }
    if (LOWORD(wParam) == IDOK ||
...
BOOL CALLBACK PluginsDlgProcA(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    lvcA.cx=210;
    lvcA.iSubItem=LVI_FUNCTION_NAME;
    SendMessage(hWndList, LVM_INSERTCOLUMNA, LVI_FUNCTION_NAME, (LPARAM)&lvcA);

    API_LoadStringA(hLangLib, STR_HOTKEY, buf, BUFFER_SIZE);
    lvcA.cx=105;
    lvcA.iSubItem=LVI_FUNCTION_HOTKEY;
    SendMessage(hWndList, LVM_INSERTCOLUMNA, LVI_FUNCTION_HOTKEY, (LPARAM)&lvcA);

    API_LoadStringA(hLangLib, STR_STATUS, buf, BUFFER_SIZE);
    lvcA.cx=63;
    lvcA.iSubItem=LVI_FUNCTION_STATUS;
    SendMessage(hWndList, LVM_INSERTCOLUMNA, LVI_FUNCTION_STATUS, (LPARAM)&lvcA);
  ...
  else if (uMsg == WM_COMMAND)
  {
    if (LOWORD(wParam) == IDC_PLUGINS_ASSIGN)
    {
      ...
      if (!pfElement)
      {
        ...
        lviA.pszText=buf;
        lviA.iItem=nSelItem;
        lviA.iSubItem=LVI_FUNCTION_HOTKEY;
...
BOOL CALLBACK PluginsDlgProcW(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  if (uMsg == WM_INITDIALOG)
  {
    ...
    lvcW.cx=210;
    lvcW.iSubItem=LVI_FUNCTION_NAME;
    SendMessage(hWndList, LVM_INSERTCOLUMNW, LVI_FUNCTION_NAME, (LPARAM)&lvcW);

    API_LoadStringW(hLangLib, STR_HOTKEY, wbuf, BUFFER_SIZE);
    lvcW.cx=105;
    lvcW.iSubItem=LVI_FUNCTION_HOTKEY;
    SendMessage(hWndList, LVM_INSERTCOLUMNW, LVI_FUNCTION_HOTKEY, (LPARAM)&lvcW);

    API_LoadStringW(hLangLib, STR_STATUS, wbuf, BUFFER_SIZE);
    lvcW.cx=63;
    lvcW.iSubItem=LVI_FUNCTION_STATUS;
    SendMessage(hWndList, LVM_INSERTCOLUMNW, LVI_FUNCTION_STATUS, (LPARAM)&lvcW);
  ...
  else if (uMsg == WM_COMMAND)
  {
    if (LOWORD(wParam) == IDC_PLUGINS_ASSIGN)
    {
      ...
      if (!pfElement)
      {
        ...
        lviW.pszText=wbuf;
        lviW.iItem=nSelItem;
        lviW.iSubItem=LVI_FUNCTION_HOTKEY;
    ...
    if (!StackInsert((stack **)&hPluginListStack.first, (stack **)&hPluginListStack.last, (stack **)&pliElement, nIndex + 1, sizeof(PLUGINLISTITEMA)))
    {
      if (pfElement=StackPluginFindA(&hPluginsStack, buf))
      {
        ...
        if (pfElement->bRunning)
        {
          ...
          lviA.pszText=buf;
          lviA.iItem=nIndex;
          lviA.iSubItem=LVI_FUNCTION_STATUS;
          SendMessage(pld->hWndList, LVM_SETITEMA, 0, (LPARAM)&lviA);
        }
      }
    ...
    if (!StackInsert((stack **)&hPluginListStack.first, (stack **)&hPluginListStack.last, (stack **)&pliElement, nIndex + 1, sizeof(PLUGINLISTITEMW)))
    {
      if (pfElement=StackPluginFindW(&hPluginsStack, wbuf))
      {
        ...
        if (pfElement->bRunning)
        {
          ...
          lviW.pszText=wbuf;
          lviW.iItem=nIndex;
          lviW.iSubItem=LVI_FUNCTION_STATUS;
          SendMessage(pld->hWndList, LVM_SETITEMW, 0, (LPARAM)&lviW);
        }
      }
...
ULONG WINAPI REOLE_AddRef(LPUNKNOWN lpTable)
{
  return ++(((REOLE *)lpTable)->uRefCount);
}

ULONG WINAPI REOLE_Release(LPUNKNOWN lpTable)
{
  return --(((REOLE *)lpTable)->uRefCount);
}
...
HRESULT WINAPI REOLE_GetNewStorage(LPUNKNOWN lpTable, LPSTORAGE FAR *lplpstg)
{
  return E_NOTIMPL;
}

HRESULT WINAPI REOLE_GetInPlaceContext(LPUNKNOWN lpTable, LPOLEINPLACEFRAME FAR *lplpFrame, LPOLEINPLACEUIWINDOW FAR *lplpDoc, LPOLEINPLACEFRAMEINFO lpFrameInfo)
{
  return E_NOTIMPL;
}

HRESULT WINAPI REOLE_ShowContainerUI(LPUNKNOWN lpTable, BOOL fShow)
{
  return E_NOTIMPL;
}

HRESULT WINAPI REOLE_QueryInsertObject(LPUNKNOWN lpTable, LPCLSID lpclsid, LPSTORAGE lpstg, LONG cp)
{
  return E_NOTIMPL;
}

HRESULT WINAPI REOLE_DeleteObject(LPUNKNOWN lpTable, LPOLEOBJECT lpoleobj)
{
  return E_NOTIMPL;
}

HRESULT WINAPI REOLE_QueryAcceptData(LPUNKNOWN lpTable, LPDATAOBJECT lpdataobj, CLIPFORMAT FAR *lpcfFormat, DWORD reco, BOOL fReally, HGLOBAL hMetaPict)
{
  *lpcfFormat=CF_TEXT;
  ...
  if (fReally)
  {
    ...
    if (lpdataobj->QueryGetData(&fmtetc) == S_OK)
    {
      if (lpdataobj->GetData(&fmtetc, &stgmed) == S_OK)
      {
...
HRESULT WINAPI REOLE_ContextSensitiveHelp(LPUNKNOWN lpTable, BOOL fEnterMode)
{
  return E_NOTIMPL;
}
