...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Stats.res" "Resources\Stats.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Stats.c Stats.res /LD /link kernel32.lib user32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
...
if exist Stats.ilk del Stats.ilk
