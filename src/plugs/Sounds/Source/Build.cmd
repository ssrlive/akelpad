...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Sounds.res" "Resources\Sounds.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Sounds.c Sounds.res /LD /link kernel32.lib user32.lib advapi32.lib comdlg32.lib winmm.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist Sounds.res del Sounds.res
if exist Sounds.obj del Sounds.obj
if exist Sounds.lib del Sounds.lib
if exist Sounds.exp del Sounds.exp
if exist Sounds.ilk del Sounds.ilk
