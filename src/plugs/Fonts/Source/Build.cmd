...
rc /R /DRC_VERSIONBIT=%BIT% /Fo"Fonts.res" "Resources\Fonts.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl /O1 %CLFLAGS% Fonts.c Fonts.res /LD /link kernel32.lib user32.lib gdi32.lib /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
