...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"FullScreen.res" "Resources\FullScreen.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% FullScreen.c FullScreen.res /LD /link kernel32.lib user32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist FullScreen.res del FullScreen.res
if exist FullScreen.obj del FullScreen.obj
if exist FullScreen.lib del FullScreen.lib
if exist FullScreen.exp del FullScreen.exp
if exist FullScreen.ilk del FullScreen.ilk
