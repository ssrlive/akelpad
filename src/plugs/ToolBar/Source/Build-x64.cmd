...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"ToolBar.res" "Resources\ToolBar.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% ToolBar.c ToolBar.res /LD /link kernel32.lib user32.lib gdi32.lib advapi32.lib comctl32.lib ole32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist ToolBar.res del ToolBar.res
if exist ToolBar.obj del ToolBar.obj
if exist ToolBar.lib del ToolBar.lib
if exist ToolBar.exp del ToolBar.exp
if exist ToolBar.ilk del ToolBar.ilk
