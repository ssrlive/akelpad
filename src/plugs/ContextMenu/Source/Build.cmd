...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"ContextMenu.res" "Resources\ContextMenu.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% ContextMenu.c ContextMenu.res /LD /link kernel32.lib user32.lib gdi32.lib comctl32.lib advapi32.lib ole32.lib shell32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
