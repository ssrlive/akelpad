...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Format.res" "Resources\Format.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Format.c Format.res /LD /link kernel32.lib user32.lib advapi32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
...
if exist Format.ilk del Format.ilk
