...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Log.res" "Resources\Log.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Log.c Log.res /LD /link kernel32.lib user32.lib shell32.lib comctl32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist Log.res del Log.res
if exist Log.obj del Log.obj
if exist Log.lib del Log.lib
if exist Log.exp del Log.exp
if exist Log.ilk del Log.ilk
