...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Exit.res" "Resources\Exit.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Exit.c Exit.res /LD /link kernel32.lib user32.lib advapi32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist Exit.res del Exit.res
if exist Exit.obj del Exit.obj
if exist Exit.lib del Exit.lib
if exist Exit.exp del Exit.exp
if exist Exit.ilk del Exit.ilk
