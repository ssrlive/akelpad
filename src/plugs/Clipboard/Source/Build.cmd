...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Clipboard.res" "Resources\Clipboard.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Clipboard.c Clipboard.res /LD /link kernel32.lib user32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist Clipboard.res del Clipboard.res
if exist Clipboard.obj del Clipboard.obj
