...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"Hotkeys.res" "Resources\Hotkeys.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% Hotkeys.c Hotkeys.res /LD /link kernel32.lib user32.lib gdi32.lib advapi32.lib comctl32.lib %LINKFLAGS% /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:DllMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist Hotkeys.res del Hotkeys.res
if exist Hotkeys.obj del Hotkeys.obj
if exist Hotkeys.lib del Hotkeys.lib
if exist Hotkeys.exp del Hotkeys.exp
if exist Hotkeys.ilk del Hotkeys.ilk
