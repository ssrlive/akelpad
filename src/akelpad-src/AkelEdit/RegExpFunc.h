...
__inline INT_PTR PatCompile(STACKREGROUP *hStack, const wchar_t *wpPat, const wchar_t *wpMaxPat)
{
...
__inline BOOL PatExec(STACKREGROUP *hStack, REGROUP *lpREGroupItem, const wchar_t *wpStr, const wchar_t *wpMaxStr)
{
...
__inline int PatStrChar(const wchar_t *wpStr, const wchar_t *wpMaxStr, int *nChar)
{
  if (*wpStr == L'\n')
  {
    *nChar=-AELB_N;
...
__inline DWORD PatCharCmp(const wchar_t **wppPat, int nStrChar, DWORD dwFlags, int *lpnPatChar)
{
...
__inline BOOL PatIsCharDelim(int nChar, const wchar_t *wpDelim, const wchar_t *wpMaxDelim)
{
  if (nChar < 0) nChar=L'\n';
...
__inline BOOL PatIsNextCharDelim(const wchar_t *wpChar, STACKREGROUP *hStack)
{
  int nNextChar;
...
__inline BOOL PatIsPrevCharDelim(const wchar_t *wpChar, STACKREGROUP *hStack)
{
  int nPrevChar;
...
__inline BOOL PatIsCharBoundary(const wchar_t *wpChar, STACKREGROUP *hStack)
{
...
__inline int PatRefIndex(const wchar_t **wppPat)
{
...
__inline INT_PTR PatStrCmp(const wchar_t *wpStrStart1, const wchar_t *wpStrEnd1, DWORD dwFlags, const wchar_t *wpStrStart2, const wchar_t *wpMaxStr)
{
...
__inline REGROUP* PatCloseGroups(REGROUP *lpREGroupItem, const wchar_t *wpPatEnd, const wchar_t *wpPatRight, REGROUP **lppREGroupNextAuto)
{
...
__inline BOOL PatIsInNonCapture(REGROUP *lpREGroupItem)
{
  do
  {
    if (lpREGroupItem->dwFlags & (REGF_POSITIVEFORWARD|REGF_NEGATIVEFORWARD))
...
__inline REGROUP* PatGetGroup(STACKREGROUP *hStack, int nIndex)
{
  REGROUP *lpREGroupItem;
...
__inline REGROUP* PatGetMatchedGroup(STACKREGROUP *hStack, int nIndex)
{
...
__inline REGROUP* PatNextGroup(REGROUP *lpREGroupItem)
{
  if (lpREGroupItem->firstChild)
...
__inline REGROUP* PatNextGroupNoChild(REGROUP *lpREGroupItem)
{
  do
  {
    if (lpREGroupItem->next)
...
__inline REGROUP* PatNextGroupForExec(REGROUP *lpREGroupItem)
{
...
__inline REGROUP* PatPrevGroup(REGROUP *lpREGroupItem)
{
...
__inline INT_PTR PatGroupStr(PATGROUPSTR *pgs)
{
  REGROUP *lpREGroupRef;
...
__inline void PatFree(STACKREGROUP *hStack)
{
...
#ifdef __AKELEDIT_H__
__inline BOOL AE_PatExec(STACKREGROUP *hStack, REGROUP *lpREGroupItem, AECHARINDEX *ciInput, const AECHARINDEX *ciMaxInput)
{
  REGROUP *lpREGroupNext;
  REGROUP *lpREGroupNextNext;
  REGROUP *lpREGroupRef;
...
__inline int AE_PatStrChar(const AECHARINDEX *ciChar)
{
  return AEC_CharAtIndex(ciChar);
}

__inline BOOL AE_PatIsNextCharDelim(const AECHARINDEX *ciChar, STACKREGROUP *hStack)
{
...
__inline BOOL AE_PatIsPrevCharDelim(const AECHARINDEX *ciChar, STACKREGROUP *hStack)
{
...
__inline BOOL AE_PatIsCharBoundary(const AECHARINDEX *ciChar, STACKREGROUP *hStack)
{
...
__inline AELINEDATA* AE_PatNextChar(AECHARINDEX *ciChar)
{
...
__inline INT_PTR AE_PatStrCmp(const AECHARINDEX *ciStrStart1, const AECHARINDEX *ciStrEnd1, DWORD dwFlags, const AECHARINDEX *ciStrStart2, AECHARINDEX *ciStrEnd2, const AECHARINDEX *ciMaxStr)
{
...
__inline INT_PTR AE_PatStrSub(const AECHARINDEX *ciChar1, const AECHARINDEX *ciChar2)
{
...
__inline REGROUP* AE_PatNextGroupForExec(REGROUP *lpREGroupItem)
{
  //No childrens, no REGF_OR
  REGROUP *lpREGroupNext=lpREGroupItem;
...
__inline void AE_PatReset(STACKREGROUP *hStack)
{
  REGROUP *lpREGroupItem=hStack->first;
...
__inline int PatStructExec(PATEXEC *pe)
{
...
__inline void PatStructFree(PATEXEC *pe)
{
  if (pe->lpREGroupStack)
  {
...
__inline INT_PTR PatReplace(PATREPLACE *pr)
{
  PATEXEC pe;
  PATEXECPARAM pep;
...
__inline int CALLBACK PatReplaceCallback(PATEXEC *pe, REGROUP *lpREGroupRoot, BOOL bMatched)
{
...
#ifdef __AKELEDIT_H__
__inline int CALLBACK AE_PatReplaceCallback(PATEXEC *pe, REGROUP *lpREGroupRoot, BOOL bMatched)
{
  PATEXECPARAM *pep=(PATEXECPARAM *)pe->lParam;
  REGROUP *lpREGroupRef;
  const wchar_t *wpRep=pep->wpRep;
...
__inline INT_PTR AE_PatStrCopy(AECHARINDEX *ciStart, AECHARINDEX *ciEnd, wchar_t *wszTarget, const wchar_t *wpTargetMax)
{
  AECHARINDEX ciCount;
...
//// Global variables
#ifdef __AKELEDIT_H__
_declspec(selectany) AECHARINDEX RegExpGlobal_ciMaxStr={0x7FFFFFFF, NULL, 0};
#endif
