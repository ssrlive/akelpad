...
rc %RCFLAGS% /R /DRC_EXEVERSION /DRC_VERSIONLANGID=%LANGID% /DRC_VERSIONBIT=%BIT% /Fo"AkelPad.res" "AkelFiles\Langs\Resources\%LANGNAME%.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% /DRC_VERSIONLANGID=%LANGID% AkelPad.c Edit.c AkelPad.res /link kernel32.lib user32.lib gdi32.lib advapi32.lib shell32.lib comctl32.lib comdlg32.lib oleaut32.lib uuid.lib version.lib %LINKFLAGS% /SUBSYSTEM:WINDOWS /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:_WinMain /OUT:AkelPad.exe
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
