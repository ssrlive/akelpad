...
rc %RCFLAGS% /R /DRC_VERSIONBIT=%BIT% /Fo"AkelUpdaterHelp.res" "Resources\AkelUpdaterHelp.rc"
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%
cl %CLFLAGS% AkelUpdaterHelp.c AkelUpdaterHelp.res /link kernel32.lib user32.lib %LINKFLAGS% /SUBSYSTEM:WINDOWS /OPT:NOWIN98 /MACHINE:%MACHINE% /NODEFAULTLIB /ENTRY:_WinMain
if not %ERRORLEVEL% == 0 set EXITCODE=%ERRORLEVEL%

::### Clean up ###::
if exist AkelUpdaterHelp.obj del AkelUpdaterHelp.obj
if exist AkelUpdaterHelp.res del AkelUpdaterHelp.res
if exist AkelUpdaterHelp.ilk del AkelUpdaterHelp.ilk
